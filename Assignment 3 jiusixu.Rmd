...
```{r}
df<-read.table("HUDK4050_2017_Cluster_Survey.csv", sep = ",", header = TRUE)

df$cat = ifelse (df$cat =="Yes",1,0)
df$gif.jiff = ifelse (df$gif.jiff =="g-iff",1,0)

df1<- dplyr::select(df, 3:7,9:10)
df1<- na.omit(df1)
df1<-scale(df1)
...
df2 <- data.frame(df1, fit1$cluster)

names(df2) <- c("1", "2", "3", "4", "5","6","7", "cluster") 


#########
df3 <- tidyr::gather(df2, "live.nyc", "movies", 1:5)
df3 <- df3 %>% group_by(live.nyc, cluster)
df3 <- summarise(df3, avg = mean(movies))

df3$live.nyc <- as.numeric(df3$live.nyc)
...
ggplot(df3, aes(live.nyc, avg, colour = cluster)) + geom_line() + xlab("live.nyc") + ylab("avg movies")
